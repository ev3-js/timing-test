...
function writeDataSync () {
  ...
  responses.push(irSync.write('mode', 'IR_SEEK'))
  responses.push(sonicSync.write('mode', 'US_DIST_CM'))
  responses.push(touchSync.write('mode', 'TOUCH'))
  responses.push(colorSync.write('mode', 'COL_REFLECT'))
...
function test () {
  ...
  readDataAsync(function (r) {
    if (r.length === 6) {
      ...
      writeDataAsync(function (r) {
        if (r.length === 6) {
          console.timeEnd('write async')
        }
      })
    }
  })
